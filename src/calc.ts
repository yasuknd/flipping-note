...
export function calcBreakEvenPrice(
  ...
  const rate = toFeeRate(feeRatePercent)
  if (rate >= 1) return null
  return (effectiveCost + saleShipping) / (1 - rate)
}
...
export function calcRecommendedPrice(
  ...
  const rate = toFeeRate(feeRatePercent)
  if (rate >= 1) return null
  const raw = (effectiveCost + saleShipping + minProfit) / (1 - rate)
  return roundUpTo100(raw)
}

export function calcFee(salePrice: number, feeRatePercent: number): number {
  return salePrice * toFeeRate(feeRatePercent)
}

/**
 * 売上金 = 販売価格 - 販売手数料 - 販売送料
 * 手元に残る販売側の入金額（仕入原価控除前）
 */
export function calcPayout(
  ...
  return salePrice - calcFee(salePrice, feeRatePercent) - saleShipping
}
...
export function calcProfit(
  ...
  return salePrice - calcFee(salePrice, feeRatePercent) - saleShipping - effectiveCost
}
